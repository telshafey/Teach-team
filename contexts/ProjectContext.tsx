...
import { Project, Task, TaskAttachment, TaskComment, ProjectFormData, SuggestedTask, TeamMember } from '../types';
...
export interface ProjectContextType {
  handleAddTask: (taskData: Omit<Task, 'id' | 'approvalStatus' | 'creatorId'>, projectId?: string) => Promise<void>;
  handleUpdateTask: (taskData: Partial<Task> & { id: string }) => Promise<void>;
  handleDeleteTask: (task: Task) => Promise<void>;
  handleUpdateProject: (projectData: Partial<Project> & { id: string }) => Promise<void>;
  handleAddProject: (projectData: ProjectFormData, suggestedTasks?: SuggestedTask[]) => Promise<Project>;
...
export const ProjectProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const { taskAttachments, handleAddTaskAttachment, handleDeleteTaskAttachment } = useTaskAttachments(initialAttachments, supabaseClient, addToast);
  const { taskComments, handleAddTaskComment, handleDeleteTaskComment } = useTaskComments(initialComments, supabaseClient, currentUser, teamMembers, addToast);

  const handleAddTask = useCallback(async (taskData: Omit<Task, 'id'| 'approvalStatus' | 'creatorId'>, projectId?: string) => {
    if (!supabaseClient || !currentUser) return;
    const fullTaskData = { ...taskData, projectId, creatorId: currentUser.id, approvalStatus: 'pending' as const };
    try {
  ...
  const handleUpdateTask = useCallback(async (taskData: Partial<Task> & { id: string }) => {
    if (!supabaseClient) return;
    try {
  ...
  const handleUpdateProject = useCallback(async (projectData: Partial<Project> & { id: string }) => {
    if (!supabaseClient) return;
    try {
  ...
  const handleAddProject = useCallback(async (projectData: ProjectFormData, suggestedTasks: SuggestedTask[] = []) => {
    ...
    try {
        const newProject = await api.insert<Project>(supabaseClient, 'projects', { ...projectData, creatorId: currentUser.id });
        ...
        if (suggestedTasks.length > 0) {
            ...
            addToast(`${suggestedTasks.length} tasks were added to the project.`, 'info');
            queryClient.invalidateQueries({ queryKey: ['tasks'] });
        }
        return newProject;
    } catch (error: any) {
