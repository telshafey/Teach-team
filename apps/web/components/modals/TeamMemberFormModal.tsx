          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {isFreelancer ? (
               <div>
                ...
                <input type="number" value={formData.hourlyRate || ''} onChange={e => setFormData({...formData, hourlyRate: e.target.value ? Number(e.target.value) : undefined})} className="w-full p-2 border rounded-md" />
              </div>
            ) : (
              <>
                  <div>
                      <label className="block text-sm font-medium">الراتب الشهري ({currency})</label>
                      <input type="number" value={formData.salary || ''} onChange={e => setFormData({...formData, salary: e.target.value ? Number(e.target.value) : undefined})} className="w-full p-2 border rounded-md" />
                  </div>
                   <div>
                      <label className="block text-sm font-medium">ساعات العمل الأسبوعية</label>
                      <input type="number" value={formData.weeklyHoursRequirement || ''} onChange={e => setFormData({...formData, weeklyHoursRequirement: e.target.value ? Number(e.target.value) : undefined})} className="w-full p-2 border rounded-md" />
                  </div>
              </>
            )}
