...
const MeetingRoom = lazy(() =>
  import("./components/meetings/MeetingRoom").then((module) => ({
    default: module.MeetingRoom,
  })),
);
...
export const AppContent: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const unsubTasks = subscribe("tasks", (payload) => {
      if (payload.eventType === "INSERT") {
        if (
          payload.new.assignedTo === currentUser.id ||
        ...
        if (isAssigned && payload.old.status !== payload.new.status) {
          ...
          addToast(
            `تغيرت حالة المهمة "${payload.new.title}" إلى ${statusMap[payload.new.status] || payload.new.status}`,
            "info",
          );
        }
      }
  ...
  const { view: currentView, props: viewProps } = useMemo(
