...
const getReportIds = (
  ...
  while (queue.length > 0) {
    ...
    const directReports = allMembers.filter(
      (m) => m.reportsTo === currentManagerId,
    );
...
export const usePendingApprovals = () => {
  ...
  const { myTeamIds, isManager } = useMemo(() => {
    ...
    if (!isManagerRole)
      return { myTeamIds: new Set<number>(), isManager: false };
  ...
  const pendingItems = useMemo((): DecisionItem[] => {
    ...
    const allPendingOvertime = overtimeRequests.filter(
      (r) => r.status === "pending",
    );
    const allPendingLeave = leaveRequests.filter((r) => r.status === "pending");
    const allPendingExpenseClaims = expenseClaims.filter(
      (r) => r.status === "pending",
    );
    const allPendingContractChanges = workContractChangeRequests.filter(
      (r) => r.status === "pending",
    );
    ...
    if (isManager) {
      const managerPendingTasks = allPendingTasks.filter(
        (t) => t.assignedTo && myTeamIds.has(t.assignedTo),
      );
      const managerPendingPlans = allPendingPlans.filter((m) =>
        myTeamIds.has(m.id),
      );

      // Managers approve things for their reports
      const managerPendingOvertime = allPendingOvertime.filter((r) =>
        myTeamIds.has(r.teamMemberId),
      );
      const managerPendingLeave = allPendingLeave.filter((r) =>
        myTeamIds.has(r.teamMemberId),
      );
      const managerPendingExpenseClaims = allPendingExpenseClaims.filter((r) =>
        myTeamIds.has(r.teamMemberId),
      );
      const managerPendingContractChanges = allPendingContractChanges.filter(
        (r) => myTeamIds.has(r.teamMemberId),
      );
      const managerPendingPenalties = allPendingPenalties.filter((p) =>
        myTeamIds.has(p.teamMemberId),
      );
