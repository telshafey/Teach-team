...
export const TimeManagementProvider: React.FC<{ children: ReactNode }> = ({
  children,
}) => {
  // State from TimeTrackingProvider
  const [activeTimer, setActiveTimer] = useState<ActiveTimer | null>(null);
  const [showLogModalFor, setShowLogModalFor] = useState<LogToCreate | null>(
    null,
  );
  ...
  const startTimer = useCallback(
    (taskId: string, taskTitle: string, projectId: string) => {
      if (activeTimer) {
        const confirmSwitch = window.confirm(
          `يوجد موقت آخر قيد التشغيل للمهمة "${activeTimer.taskTitle}". هل تريد إيقافه وبدء موقت جديد؟ (لن يتم حفظ الوقت)`,
        );
        if (!confirmSwitch) {
          return;
        }
...
export const useTimeManagement = () => {
  ...
  if (context === undefined) {
    throw new Error(
      "useTimeManagement must be used within a TimeManagementProvider",
    );
  }
  return context;
};
