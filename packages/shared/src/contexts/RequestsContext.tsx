...
export const RequestsProvider: React.FC<{ children: ReactNode }> = ({
  ...
  const { data: leaveRequests = [], isLoading: isLoadingLR } = useQuery<
    LeaveRequest[]
  >({
  ...
  const { data: overtimeRequests = [], isLoading: isLoadingOR } = useQuery<
    OvertimeRequest[]
  >({
  ...
  const { data: expenseClaims = [], isLoading: isLoadingEC } = useQuery<
    ExpenseClaim[]
  >({
  ...
  const { data: workContractChangeRequests = [], isLoading: isLoadingWCR } =
    useQuery<WorkContractChangeRequest[]>({
      queryKey: ["work_contract_change_requests"],
      queryFn: () =>
        api.getAll(supabaseClient!, "work_contract_change_requests"),
      enabled,
    });
  ...
  const isLoading =
    isLoadingLR || isLoadingOR || isLoadingEC || isLoadingWCR || isLoadingP;
  ...
  useEffect(() => {
    ...
    const unsubs = Object.keys(tableToQueryKey).map((table) =>
      subscribe(table, handleTableChange),
    );
  ...
  const updateRequest = async <T extends { id: string }>(
    ...
    try {
      const updatedItem = await api.update<T>(
        supabaseClient,
        table,
        id,
        updates,
  ...
  const submitLeaveRequest = async (formData: LeaveRequestFormData) => {
    ...
    try {
      await api.insert<LeaveRequest>(supabaseClient, "leave_requests", {
        ...formData,
  ...
  const handleUpdateLeaveStatus = async (
    ...
    await updateRequest<LeaveRequest>("leave_requests", id, {
      status,
      managerNotes,
    });
  };
  ...
  const submitOvertimeRequest = async (formData: OvertimeRequestFormData) => {
    if (!supabaseClient || !currentUser) return;
    try {
      await api.insert<OvertimeRequest>(supabaseClient, "overtime_requests", {
        ...formData,
        teamMemberId: currentUser.id,
        status: "pending",
      });
  ...
  const handleUpdateOvertimeStatus = async (
    ...
    await updateRequest<OvertimeRequest>("overtime_requests", id, {
      status,
      managerNotes,
    });
  };
  ...
  const handleSubmitExpenseClaim = async (
    ...
    if (!supabaseClient) return;
    try {
      await api.insert<ExpenseClaim>(supabaseClient, "expense_claims", {
        ...formData,
        status: "pending",
      });
  ...
  const handleUpdateExpenseClaimStatus = async (
    id: string,
    status: ExpenseClaim["status"],
  ) => {
  ...
  const submitWorkContractChangeRequest = async (
    formData: WorkContractChangeRequestFormData,
  ) => {
    if (!supabaseClient || !currentUser) return;
    try {
      ...
      await api.insert<WorkContractChangeRequest>(
        supabaseClient,
  ...
  const handleUpdateWorkContractChangeRequestStatus = async (
    id: string,
    status: WorkContractChangeRequest["status"],
    notes: string,
    modifications?: { hours: number; salary: number },
    ...
    const updates: Partial<WorkContractChangeRequest> = {
      status,
      managerNotes: notes,
    };
    ...
    await updateRequest<WorkContractChangeRequest>(
      "work_contract_change_requests",
      id,
      updates,
    );
  ...
  const handleIssuePenalty = async (formData: PenaltyFormData) => {
    if (!supabaseClient || !currentUser) return;
    try {
      const newPenaltyData: Omit<Penalty, "id" | "createdAt"> = {
        ...formData,
  ...
  const handleAppealPenalty = async (id: string, appealReason: string) => {
    await updateRequest<Penalty>("penalties", id, {
      status: "appealed",
      appealReason,
    });
  };
  const handleUpdatePenaltyStatus = async (
    id: string,
    status: Penalty["status"],
    notes: string,
  ) => {
