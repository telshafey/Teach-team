...
export const MeetingProvider: React.FC<{ children: ReactNode }> = ({
  ...
  const handleAddMeeting = useCallback(
    async (meetingData: MeetingFormData) => {
      ...
      if (!defaultRoom) {
        addToast(
          "فشل جدولة الاجتماع. لم يتم تكوين غرفة الاجتماعات الافتراضية في الإعدادات.",
          "error",
        );
      ...
      const endTime = new Date(
        startTime.getTime() + meetingData.duration * 60000,
      );
    ...
    async (meetingId: string) => {
      ...
      try {
        ...
        if (!currentAttendees.includes(currentUser.id)) {
          ...
          await api.update<Meeting>(supabaseClient, "meetings", meetingId, {
