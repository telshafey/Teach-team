...
export const SupportProvider: React.FC<{ children: ReactNode }> = ({
  children,
}) => {
  ...
  >({
    ...
    queryFn: async () => {
      const data = await api.getAll<SupportTicket>(
        supabaseClient!,
        "support_tickets",
      );
      return data.sort(
  ...
  useEffect(() => {
    const handleTableChange =
      <T extends { id: string }>(queryKey: string) =>
    ...
    const unsubTickets = subscribe(
      "support_tickets",
      handleTableChange<SupportTicket>("support_tickets"),
    );
    const unsubComments = subscribe(
      "ticket_comments",
      handleTableChange<TicketComment>("ticket_comments"),
    );
  ...
  const createTicket = useCallback(
    async (
      ...
      try {
        ...
        await api.insert<SupportTicket>(
          supabaseClient,
          "support_tickets",
          newTicketData,
        );
  ...
  const updateTicket = useCallback(
    async (id: string, updates: Partial<SupportTicket>) => {
      ...
      try {
        await api.update<SupportTicket>(supabaseClient, "support_tickets", id, {
          ...updates,
  ...
  const addComment = useCallback(
    async (data: Omit<TicketComment, "id" | "authorId" | "createdAt">) => {
      if (!supabaseClient || !currentUser) return;
      try {
        ...
        await api.insert<TicketComment>(
          supabaseClient,
          "ticket_comments",
          newCommentData,
        );
