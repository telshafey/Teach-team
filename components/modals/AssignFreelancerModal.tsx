...
export const FreelancerContractModal: React.FC<
  FreelancerContractModalProps
> = ({ isOpen, onClose, onSave, project }) => {
  ...
  useEffect(() => {
    if (billingType === "hourly" && freelancerId) {
      const selectedFreelancer = teamMembers.find(
        (m) => m.id === parseInt(freelancerId),
      );
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      const contractData: Omit<FreelancerContract, "status" | "notes"> = {
        ...
        hourlyRate:
          billingType === "hourly" ? parseFloat(hourlyRate) : undefined,
      };
    ...
    >
      <div className="bg-white dark:bg-slate-800 rounded-lg shadow-xl p-6 w-full max-w-lg">
        <h2 className="text-xl font-bold mb-4 text-slate-800 dark:text-slate-100">
          {isEditing ? "تعديل عقد المستقل" : "تعيين مستقل وعقد"}
        </h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex space-x-4 rtl:space-x-reverse">
              ...
              <label className="flex items-center space-x-2 rtl:space-x-reverse">
                <input
                  ...
                  name="billingType"
                  value="per-task"
                  checked={billingType === "per-task"}
                  onChange={() => setBillingType("per-task")}
                  className="h-4 w-4 text-sky-600 focus:ring-sky-500"
                />
                <span>بالقطعة</span>
              </label>
          ...
          {billingType === "fixed" && (
            <div>
              <label
                ...
                className="block text-sm font-medium text-slate-600 dark:text-slate-300 mb-1"
              >
                المبلغ الإجمالي للمشروع ({currency})
              </label>
              <input
                type="number"
                step="1"
          ...
          <div className="flex justify-end space-x-2 rtl:space-x-reverse pt-4">
            ...
            <button
              type="submit"
              disabled={isSaving || !freelancerId}
