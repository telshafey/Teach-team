...
export const DailyLogDetailModal: React.FC<DailyLogDetailModalProps> = ({ isOpen, onClose, date, logs, onAdd, onEdit, onDelete, isEditable }) => {
  ...
  const confirmDelete = async () => {
    if (logToDelete) {
        ...
        // Check if this was the last log, if so, close the detail modal too
        if (logs.length === 1) {
            onClose();
        }
        setLogToDelete(null);
    ...
    <>
                  ...
                  <div key={log.id} className="p-3 bg-slate-50 dark:bg-slate-700/50 rounded-md flex justify-between items-start group">
                      ...
                      {isEditable && (
                          <div className="flex items-center space-x-2 rtl:space-x-reverse opacity-0 group-hover:opacity-100 transition-opacity">
                              ...
                              <button onClick={() => handleDeleteClick(log)} className="p-1.5 text-slate-500 hover:text-red-600" aria-label="حذف السجل"><TrashIcon className="w-4 h-4" /></button>
                          </div>
                      )}
                  </div>
      ...
      {isConfirmOpen && (
          <ConfirmationModal
            ...
            message="هل أنت متأكد من رغبتك في حذف هذا السجل الزمني؟ لا يمكن التراجع عن هذا الإجراء."
