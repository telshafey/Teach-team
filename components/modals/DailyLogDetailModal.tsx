          ...
          <div>
            ...
            {!isEditable && (
              <p className="text-xs text-amber-600 dark:text-amber-400 font-semibold mt-1">
                انتهت الفترة المسموح بها لتعديل سجلات هذا اليوم.
              </p>
            )}
          </div>
          {isEditable && (
            <button
              onClick={onAdd}
              className="flex items-center space-x-2 rtl:space-x-reverse px-3 py-1.5 text-sm font-semibold text-white bg-sky-600 rounded-md hover:bg-sky-700"
            >
        ...
        <div className="space-y-3 max-h-96 overflow-y-auto pr-2">
          {logs.length > 0 ? (
            logs.map((log) => (
              <div
                key={log.id}
                className="p-3 bg-slate-50 dark:bg-slate-700/50 rounded-md flex justify-between items-start group"
              >
                <div>
                  <div className="flex items-center space-x-3 rtl:space-x-reverse">
                    <span className="text-sm font-bold text-sky-600 dark:text-sky-300 bg-sky-100 dark:bg-sky-900/50 px-2 py-0.5 rounded-full">
                      {log.hours} ساعات
                    </span>
                    <p className="font-semibold text-slate-700 dark:text-slate-200">
                      {projectsMap[log.projectId || ""] || "عمل آخر"}
                    </p>
                  </div>
