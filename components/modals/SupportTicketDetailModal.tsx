...
export const SupportTicketDetailModal: React.FC<
  ...
  return (
    ...
    >
      <div className="bg-white dark:bg-slate-800 rounded-lg shadow-xl p-6 w-full max-w-3xl max-h-[90vh] flex flex-col">
        <h2 className="text-xl font-bold mb-2 text-slate-800 dark:text-slate-100 flex-shrink-0">
          {ticket.subject}
        </h2>
        <div className="flex items-center space-x-4 rtl:space-x-reverse text-sm text-slate-500 mb-4 flex-shrink-0">
          ...
          <span>
            في:{" "}
            {format(parseISO(ticket.createdAt), "d MMM yyyy", { locale: arSA })}
          </span>
        </div>

        <div className="flex-1 overflow-y-auto pr-2 space-y-6">
          <div className="p-4 bg-slate-50 dark:bg-slate-700/50 rounded-md text-sm whitespace-pre-wrap">
            {ticket.description}
          </div>
            ...
            {ticketComments.map((comment) => {
              ...
              return (
                ...
                >
                  ...
                  />
                  <div
                    className={`flex-1 ${comment.isInternal ? "bg-amber-50 dark:bg-amber-900/30 border-amber-300" : "bg-slate-100 dark:bg-slate-700"} rounded-lg p-3`}
                  >
        ...
        <div className="mt-4 pt-4 border-t flex-shrink-0">
          ...
          <form onSubmit={handleCommentSubmit} className="space-y-2">
            ...
            <div className="flex justify-between items-center">
              ...
              <button
                type="submit"
                className="px-4 py-2 text-sm font-semibold text-white bg-sky-600 rounded-md ml-auto"
              >
                إرسال
              </button>
            </div>
