...
export const DecisionDetailModal: React.FC<DecisionDetailModalProps> = ({
  ...
  const details = useMemo(() => {
    ...
    if (isTeamMember(item))
      return <DetailRow label="Employee" value={item.name} />;
    if (isOvertimeRequest(item))
      return (
        <>
          <DetailRow label="Employee" value={member?.name || ""} />
          <DetailRow label="Hours" value={`${item.requestedHours} hours`} />
        </>
      );
    if (isLeaveRequest(item))
      return (
        <>
          ...
          <DetailRow
            label="Dates"
            value={`${format(parseISO(item.startDate), "d MMM")} - ${format(parseISO(item.endDate), "d MMM")}`}
          />
    ...
    if (isExpenseClaim(item))
      return (
        <>
          <DetailRow label="Employee" value={member?.name || ""} />
          <DetailRow label="Amount" value={`${item.amount} ${currency}`} />
    ...
    if (isWorkContractChangeRequest(item))
      return (
        <>
          ...
          <DetailRow
            label="Current"
            value={`${item.currentWeeklyHours}h/week, ${item.currentSalary} ${currency}/month`}
          />
          <DetailRow
            label="Requested"
            value={`${item.requestedWeeklyHours}h/week, ${item.requestedSalary} ${currency}/month`}
          />
          <DetailRow label="Reason" value={item.reason} />
        </>
      );
  ...
  const handleDecision = async (
    status: "approved" | "rejected" | "needs-adjustment",
  ) => {
    if (!item) return;
    setIsSaving(true);
    try {
      ...
      if (isWorkContractChangeRequest(item)) {
        const modifications =
          status === "approved"
            ? { hours: approvedHours, salary: approvedSalary }
            : undefined;
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center">
      <div className="bg-white dark:bg-slate-800 rounded-lg p-6 w-full max-w-lg">
        ...
        {isWorkContractChangeRequest(item) && (
          <div className="p-3 bg-slate-100 rounded-md space-y-2 mb-4">
            <h4 className="font-semibold text-sm">
              Approve with Modifications (Optional)
            </h4>
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="text-xs">Approved Hours</label>
                <input
                  type="number"
                  value={approvedHours}
              ...
              <div>
                <label className="text-xs">Approved Salary</label>
                <input
                  type="number"
                  value={approvedSalary}
                  onChange={(e) => setApprovedSalary(Number(e.target.value))}
                  className="w-full p-1 border rounded"
                />
              </div>
            </div>
        ...
        <div className="flex justify-end space-x-2 mt-4">
          ...
          {isTask(item) && (
            <button
              onClick={() => handleDecision("needs-adjustment")}
