...
export const WorkContractChangeRequestModal: React.FC<
  WorkContractChangeRequestModalProps
        ...
        <div className="bg-white dark:bg-slate-800 rounded-lg shadow-xl p-6 w-full max-w-lg">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-4 p-3 bg-slate-100 dark:bg-slate-700/50 rounded-md">
              <div>
                ...
                <p className="font-bold text-slate-800 dark:text-slate-100">
                  {currentUser.weeklyHoursRequirement || "غير محدد"} ساعة/أسبوع
                </p>
              </div>
              <div>
                <label className="block text-xs font-medium text-slate-500 dark:text-slate-400">
                ...
                <p className="font-bold text-slate-800 dark:text-slate-100">
                  {currentUser.salary?.toLocaleString() || "غير محدد"}{" "}
                  {currency}
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      requestedWeeklyHours: Number(e.target.value),
                    })
                  }
              ...
              <div>
                <label
                  htmlFor="requestedSalary"
                  className="block text-sm font-medium text-slate-600 dark:text-slate-300 mb-1"
                >
                  الراتب المطلوب ({currency})
                </label>
                <input
                  type="number"
                  id="requestedSalary"
                  ...
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      requestedSalary: Number(e.target.value),
                    })
                  }
                  className="w-full p-2 border border-slate-300 dark:border-slate-600 rounded-md text-sm"
                  required
