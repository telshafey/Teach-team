...
const SlipRow: React.FC<{
  ...
  isDeduction?: boolean;
  currency: string;
}> = ({ label, value, isDeduction, currency }) => {
  ...
  return (
    ...
    >
      <div className="bg-white dark:bg-slate-800 rounded-lg shadow-xl w-full max-w-2xl max-h-[calc(var(--vh,1vh)*90)] flex flex-col">
        <div id="salary-slip-print-area" className="p-8 overflow-y-auto">
          ...
          <div className="space-y-2 text-sm">
            <div className="flex justify-between items-center py-2 border-b border-slate-200 dark:border-slate-700">
              ...
              <span className="font-semibold text-slate-800 dark:text-slate-200">
                {slipData.baseSalary.toLocaleString(undefined, {
                  minimumFractionDigits: 2,
                  maximumFractionDigits: 2,
                })}{" "}
                {currency}
            ...
            <SlipRow
              label="مستحقات ساعات إضافية"
              value={slipData.overtimePay}
              currency={currency}
            />
            <SlipRow
              label="تعويض مصروفات"
              value={slipData.expensesReimbursed}
              currency={currency}
            />
            <SlipRow
              label="خصم جزاءات"
              value={slipData.penaltiesDeducted}
              isDeduction
              currency={currency}
            />
          </div>
          ...
          <div className="flex justify-between items-center mt-6 pt-4 border-t-2 border-slate-300 dark:border-slate-600">
            ...
            <span className="text-2xl font-extrabold text-sky-600 dark:text-sky-400">
              {slipData.netSalary.toLocaleString(undefined, {
                minimumFractionDigits: 2,
                maximumFractionDigits: 2,
              })}{" "}
              {currency}
        ...
        <div
          id="report-controls"
          className="flex justify-end space-x-2 rtl:space-x-reverse p-4 border-t border-slate-200 dark:border-slate-700"
        >
          <button
            type="button"
            onClick={onClose}
          ...
          <button
            onClick={handlePrint}
            className="flex items-center space-x-2 rtl:space-x-reverse px-4 py-2 text-sm font-semibold text-white bg-sky-600 rounded-md hover:bg-sky-700"
          >
