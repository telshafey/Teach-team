...
export const RoleManagementPage: React.FC<RoleManagementPageProps> = ({
  ...
  const { roles, handleAddRole, handleUpdateRole, handleDeleteRole } =
    useTeamContext();
  ...
  const [selectedRoleId, setSelectedRoleId] = useState<string | null>(
    initialRoleId || firstEditableRole?.id || null,
  );
          ...
          <Card
            ...
            headerActions={
              <button
                ...
                className="p-1.5 text-sky-600 dark:text-sky-300 hover:bg-sky-100 dark:hover:bg-sky-900/50 rounded-full"
                  ...
                  <li key={role.id}>
                    ...
                    >
                      <div className="flex items-center space-x-2 rtl:space-x-reverse">
                        ...
                        <span
                          className={`font-medium ${selectedRoleId === role.id ? "text-sky-700 dark:text-sky-300" : "text-slate-700 dark:text-slate-200"}`}
                        >
                          {role.name}
                        </span>
                      </div>
                ...
                {PROTECTED_ROLES.includes(selectedRole.id) ? (
                  <div className="text-center p-4 bg-slate-100 dark:bg-slate-700 rounded-md">
                    ...
                    <p className="text-sm font-semibold text-slate-700 dark:text-slate-200">
                      صلاحيات هذا الدور مدمجة ولا يمكن تعديلها.
                    </p>
                    {selectedRole.id === "gm" && (
                      <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                        المدير العام لديه كل الصلاحيات بشكل افتراضي.
                      </p>
                    )}
                  </div>
                ) : (
                  Object.entries(PERMISSION_GROUPS).map(
                    ([groupName, permissions]) => (
                      <div key={groupName}>
                        <h4 className="font-semibold text-slate-800 dark:text-slate-200 mb-3 border-b border-slate-200 dark:border-slate-700 pb-2">
                          {groupName}
                        </h4>
