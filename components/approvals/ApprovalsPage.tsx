...
const categoryMap: {
  [key in ApprovalCategory]: {
    title: string;
    check: (item: DecisionItem) => boolean;
  };
} = {
  tasks: { title: "تسليمات المهام", check: isTask },
  plans: { title: "خطط العمل الأسبوعية", check: isTeamMember },
  contracts: {
    title: "عقود المستقلين",
    check: (item): item is Project =>
      isProject(item) && !!item.freelancerContract,
  },
  ...
  contractChanges: {
    title: "طلبات تعديل العقود",
    check: isWorkContractChangeRequest,
  },
...
export const ApprovalsPage: React.FC = () => {
  ...
  const groupOrder: ApprovalCategory[] = [
    "tasks",
    "plans",
    "leave",
    "overtime",
  ...
  const firstGroupWithItems = hasPendingItems
    ? groupOrder.find(
        (key) => groupedApprovals[key] && groupedApprovals[key]!.length > 0,
      )
    : undefined;
      ...
      <div className="p-6">
        <div className="mb-6">
          ...
          <p className="text-md text-slate-500 dark:text-slate-400">
            جميع الطلبات التي تحتاج إلى مراجعتك.
          </p>
        </div>
              ...
              return (
                ...
                >
                  {groupItems.map((item, itemIndex) => (
                    <ApprovalItemCard
                      key={(item as any).id ?? `${key}-${itemIndex}`}
                      item={item}
