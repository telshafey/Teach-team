...
export const ExpenseClaimsTab: React.FC<ExpenseClaimsTabProps> = ({
  onNewClaim,
}) => {
  const { expenseClaims, handleUpdateExpenseClaimStatus } =
    useRequestsContext();
  const { teamMembers, hasPermission } = useTeamContext();
  const { currency } = useSettingsContext();
  const { currentUser } = useAuth();
  const { supabaseClient } = useSupabase();
  const [statusFilter, setStatusFilter] = useState<"all" | ExpenseClaimStatus>(
    "all",
  );
    ...
    <Card>
      <div className="p-4 border-b border-slate-200 dark:border-slate-700 flex flex-col sm:flex-row justify-between items-center gap-4">
        ...
        {hasPermission("submit_expenses") && (
          <button
            onClick={onNewClaim}
            className="flex items-center space-x-2 rtl:space-x-reverse px-3 py-1.5 text-sm font-semibold text-white bg-sky-600 rounded-md hover:bg-sky-700 w-full sm:w-auto"
          >
            <PlusIcon className="w-4 h-4" />
            <span>تقديم طلب صرف</span>
          </button>
        )}
      </div>
      <div className="overflow-x-auto">
          ...
          <tbody>
            {claimsToDisplay.map((claim) => {
              const member = teamMembers.find(
                (m) => m.id === claim.teamMemberId,
              );
              const project = projects.find((p) => p.id === claim.projectId);
              return (
                <tr key={claim.id} className="border-b dark:border-slate-700 text-slate-900 dark:text-slate-100">
                      ...
                      {claim.status === "pending" && (
                        <div className="flex space-x-2 rtl:space-x-reverse">
                          <button
                            ...
                            className="p-1.5 text-green-500 hover:bg-green-100 rounded-full"
                          >
        ...
        {claimsToDisplay.length === 0 && (
          <p className="text-center text-slate-500 py-8">
            لا توجد طلبات صرف تطابق هذه الفئة.
          </p>
        )}
      </div>
    </Card>
