...
export const FinancePage: React.FC<FinancePageProps> = ({ initialView }) => {
  ...
  const { penalties, handleIssuePenalty, handleSubmitExpenseClaim } =
    useRequestsContext();
  ...
  const [reviewingItem, setReviewingItem] = useState<Project | Penalty | null>(
    null,
  );
  ...
  const navItems = [
    {
      id: "overview",
      label: "نظرة عامة",
      permission: hasPermission("view_finances"),
    },
    {
    ...
    {
      id: "freelancer",
      label: currentUser?.roleId === "freelancer" ? "عقودي" : "عقود المستقلين",
      permission: true,
    },
    { id: "expenses", label: "طلبات الصرف", permission: true },
    {
      ...
      label: "الجزاءات",
      permission:
        hasPermission("issue_penalties") || hasPermission("approve_penalties"),
    },
    {
      id: "salaries",
      label: "الرواتب",
      permission: hasPermission("view_all_salaries"),
    },
    {
      id: "salary_slips",
      label: "قسائم الرواتب",
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-md text-slate-500 dark:text-slate-400">
            نظرة عامة على التكاليف والمصروفات والجزاءات.
          </p>
        </div>
      </div>
      ...
      <div className="border-b border-slate-200 dark:border-slate-700 mb-6">
        <nav
          className="-mb-px flex space-x-6 rtl:space-x-reverse overflow-x-auto"
          aria-label="Tabs"
        >
          {navItems.map((item) => (
            <button
      ...
      {activeTab === "freelancer" && (
        <FreelancerContractsTab
          onReview={setReviewingItem as (item: Project) => void}
        />
      )}
      {activeTab === "expenses" && (
        <ExpenseClaimsTab onNewClaim={() => setIsExpenseModalOpen(true)} />
      )}
      {activeTab === "penalties" && (
        <PenaltiesTab
          penalties={penalties}
          onReview={setReviewingItem as (item: Penalty) => void}
          onNew={() => setIsPenaltyModalOpen(true)}
        />
      )}
