...
export const FreelancerContractsTab: React.FC<FreelancerContractsTabProps> = ({
  onReview,
}) => {
  ...
  const contractsToDisplay = useMemo(() => {
    if (isFreelancer) {
      return projects.filter((p) =>
        p.members?.some((m) => m.teamMemberId === currentUser.id),
      );
    }
  ...
  const handleSaveProposal = async (proposal: BillingProposalFormData) => {
    ...
    await handleUpdateProject({
      id: billingProject.id,
      freelancerContract: newContract,
    });
  };

  const handleSaveAssignment = async (
    contractData: Omit<FreelancerContract, "status" | "notes">,
  ) => {
    ...
    await handleUpdateProject({
      ...
      freelancerContract: newContract,
      members: [
        { teamMemberId: contractData.freelancerId, projectRole: "Member" },
      ],
    });
  };
          ...
          <tbody>
            {contractsToDisplay.map((project) => {
              ...
              if (contract?.type === "fixed")
                value = `${contract.amount} ${currency}`;
              if (contract?.type === "hourly")
                value = `${contract.hourlyRate} ${currency}/ساعة`;
              if (contract?.type === "per-task") value = `بالقطعة`;

              return (
                <tr key={project.id} className="border-b dark:border-slate-700 text-slate-900 dark:text-slate-100">
                  <td className="px-4 py-2 font-medium">{project.name}</td>
                  {!isFreelancer && (
                    <td className="px-4 py-2">
                      {contract ? membersMap[contract.freelancerId] : "-"}
                    </td>
                  )}
                  ...
                  <td className="px-4 py-2">
                    {isFreelancer && !contract && (
                      <button
                        onClick={() => setBillingProject(project)}
                        className="text-xs font-semibold text-sky-600"
                      >
                        اقتراح عقد
                      </button>
                    )}
                    {canManageContracts && !contract && (
                      <button
                    ...
                    {canManageContracts && contract?.status === "pending" && (
                      <button
                    ...
                    {canManageContracts && contract?.status === "approved" && (
                      <button
                        onClick={() => setAssigningProject(project)}
