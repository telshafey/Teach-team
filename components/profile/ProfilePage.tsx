...
export const ProfilePage: React.FC = () => {
    ...
    const { submitLeaveRequest, cancelLeaveRequest, submitOvertimeRequest, cancelOvertimeRequest, handleSubmitExpenseClaim, submitWorkContractChangeRequest, handleAppealPenalty } = useRequestsContext();
    ...
    const handleCancelRequest = async (id: string, type: 'leave' | 'overtime') => {
    ...
    const tabs = [
        ...
        { id: 'work_hours', label: 'ساعات العمل', show: isEmployee && currentUser?.weeklyHoursRequirement },
        { id: 'salary', label: 'تقرير الراتب', show: isEmployee && currentUser?.salary },
        ...
        <>
            <div className="p-6">
                <div className="mb-6">
                    ...
                    <p className="text-md text-slate-500 dark:text-slate-400">إدارة معلوماتك وطلباتك وإعدادات حسابك.</p>
                </div>
                    ...
                    <div className="lg:col-span-3">
                        <div className="border-b border-slate-200 dark:border-slate-700 mb-6">
                            <nav className="-mb-px flex space-x-6 rtl:space-x-reverse overflow-x-auto">
                                {tabs.map(tab => (
                                     <button key={tab.id} onClick={() => setActiveTab(tab.id as ProfileTab)} className={`whitespace-nowrap pb-4 px-1 border-b-2 font-medium text-sm ${activeTab === tab.id ? 'border-sky-500 text-sky-600' : 'border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300'}`}>
                                        {tab.label}
                                    </button>
                                ))}
                            </nav>
                        ...
                        {activeTab === 'requests' && <ProfileRequests onNewLeave={() => setIsLeaveModalOpen(true)} onNewExpense={() => setIsExpenseModalOpen(true)} onNewOvertime={() => setIsOvertimeModalOpen(true)} onCancelRequest={handleCancelRequest} />}
                        {activeTab === 'performance' && <ProfilePerformance />}
                        {activeTab === 'work_hours' && <ProfileWorkHours onStartChangeRequest={() => setIsContractChangeModalOpen(true)} />}
                        {activeTab === 'salary' && <ProfileSalaryReport />}
                        {activeTab === 'penalties' && <ProfilePenalties onAppeal={setAppealingPenalty} />}
            ...
            {isContractChangeModalOpen && <WorkContractChangeRequestModal isOpen={isContractChangeModalOpen} onClose={() => setIsContractChangeModalOpen(false)} onSave={submitWorkContractChangeRequest} />}
            {appealingPenalty && <AppealPenaltyModal isOpen={!!appealingPenalty} onClose={() => setAppealingPenalty(null)} onSave={handleSaveAppeal} />}
        </>
    );
};
