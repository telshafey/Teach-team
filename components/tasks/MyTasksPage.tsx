...
import { Task, TaskStatus, ApprovalStatus, TaskFormData } from '../../types';
...
export const MyTasksPage: React.FC = () => {
    ...
    const { projects, tasks, isLoading, handleAddTask } = useProjectContext();
                            ...
                            <tbody>
                                {myTasks.map(task => (
                                    <tr key={task.id} onClick={() => setViewingTask(task)} className="border-b dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/30 cursor-pointer">
                                        ...
                                        <td className="px-6 py-4">{task.projectId ? projectsMap[task.projectId] : 'مهمة عامة'}</td>
