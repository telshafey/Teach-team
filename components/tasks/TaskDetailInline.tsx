...
import { Task, TaskComment, TaskAttachment, TeamMember, TaskStatus, Project } from '@shared/types';
import { useTeamContext } from '@shared/contexts/TeamContext';
import { useProjectContext } from '@shared/contexts/ProjectContext';
import { useAuth } from '@shared/contexts/AuthContext';
import { PaperClipIcon, ChatBubbleLeftEllipsisIcon, ArrowUpTrayIcon, PencilIcon, TrashIcon, ArrowLeftIcon } from '../ui/Icons';
    ...
    <>
            ...
            <div>
                ...
                <div className="space-y-2">
                    {attachmentsForThisTask.map(att => (
                        <div key={att.id} className="group flex items-center justify-between p-2 bg-slate-100 dark:bg-slate-700/50 rounded-md">
                        ...
                        return (
                            <div key={comment.id} className="flex items-start space-x-3 rtl:space-x-reverse group">
                                ...
                                <div className="flex-1">
                                    ...
                                    <p className="text-sm bg-slate-100 dark:bg-slate-700 p-2 rounded-md mt-1">{comment.text}</p>
                                </div>
                            </div>
                        );
                    })}
        ...
        <div className="mt-4 pt-4 border-t dark:border-slate-800 flex-shrink-0">
            <form onSubmit={handleAddCommentSubmit} className="flex items-start space-x-3 rtl:space-x-reverse">
                ...
                <div className="flex-1">
                    <textarea value={newComment} onChange={e => setNewComment(e.target.value)} placeholder="أضف تعليقًا... (يمكنك الإشارة إلى عضو باستخدام @)" rows={1} className="w-full p-2 border border-slate-300 dark:border-slate-600 rounded-md text-sm dark:bg-slate-900" />
                    <button type="submit" className="mt-2 px-3 py-1.5 text-xs font-semibold text-white bg-sky-600 rounded-md hover:bg-sky-700">إضافة</button>
                </div>
            </form>
        </div>
    </>
...
const TaskDetailEdit: React.FC<TaskDetailEditProps> = ({
    ...
    <form onSubmit={handleFormSubmit} className="flex-1 pr-2 space-y-6 mt-4">
        <div>
            <label className="block text-sm font-medium mb-1 dark:text-slate-300">عنوان المهمة</label>
            <input type="text" value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} className="w-full p-2 border border-slate-300 dark:border-slate-600 rounded-md text-sm dark:bg-slate-900" required />
        </div>
        <div>
            <label className="block text-sm font-medium mb-1 dark:text-slate-300">المشروع</label>
            <select value={formData.projectId} onChange={e => setFormData({...formData, projectId: e.target.value})} className="w-full p-2 border border-slate-300 dark:border-slate-600 rounded-md text-sm dark:bg-slate-900" disabled={isProjectFixed}>
        ...
        <div className="grid grid-cols-2 gap-4">
            <div>
                <label className="block text-sm font-medium mb-1 dark:text-slate-300">مسندة إلى</label>
                <select 
            ...
            <div>
                <label className="block text-sm font-medium mb-1 dark:text-slate-300">تاريخ الاستحقاق</label>
                <input type="date" value={formData.dueDate} onChange={e => setFormData({...formData, dueDate: e.target.value})} className="w-full p-2 border border-slate-300 dark:border-slate-600 rounded-md text-sm dark:bg-slate-900" />
            </div>
        </div>
        <div>
            <label className="block text-sm font-medium mb-1 dark:text-slate-300">الحالة</label>
            <select value={formData.status} onChange={e => setFormData({...formData, status: e.target.value as TaskStatus})} className="w-full p-2 border border-slate-300 dark:border-slate-600 rounded-md text-sm dark:bg-slate-900">
        ...
        <div className="flex justify-start space-x-2 rtl:space-x-reverse pt-4 border-t dark:border-slate-800">
            <button type="submit" disabled={isSaving} className="px-6 py-2.5 text-sm font-semibold text-white bg-sky-600 hover:bg-sky-700 rounded-md disabled:bg-slate-400">
                {isSaving ? <LoadingSpinner/> : 'حفظ'}
            </button>
             <button type="button" onClick={onCancel} className="px-6 py-2.5 text-sm font-semibold text-slate-700 dark:text-slate-300 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 rounded-md border border-slate-200 dark:border-slate-600">إلغاء</button>
        </div>
    </form>
);
...
export const TaskDetailInline: React.FC<TaskDetailInlineProps> = ({ onClose, task, onSave, initialMode = 'view', projectId, isProjectFixed }) => {
      ...
      <div className="flex flex-col">
          <div className="flex justify-between items-center bg-slate-50 dark:bg-slate-800/50 p-4 border-b dark:border-slate-700">
              <div className="flex items-center space-x-3 rtl:space-x-reverse">
                 <button onClick={onClose} className="p-2 text-slate-500 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-full transition-colors flex-shrink-0">
              ...
              <div className="flex items-center space-x-2">
                   {!isNew && !isEditing && onSave && <button onClick={() => setIsEditing(true)} className="p-2 text-slate-500 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-full"><PencilIcon className="w-5 h-5"/></button>}
              </div>
          </div>
