...
const COLUMNS: { id: TaskStatus; title: string; color: string }[] = [
    { id: 'todo', title: 'قيد الانتظار', color: 'border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-800/50' },
    { id: 'inprogress', title: 'قيد التنفيذ', color: 'border-sky-300 dark:border-sky-800 bg-sky-50 dark:bg-sky-900/20' },
    { id: 'done', title: 'مكتملة', color: 'border-green-300 dark:border-green-800 bg-green-50 dark:bg-green-900/20' },
];

export const TaskKanbanBoard: React.FC<TaskKanbanBoardProps> = ({ tasks, onTaskClick, onUpdateTaskStatus, membersMap }) => {
                ...
                >
                    ...
                    <div className="flex-1 space-y-3 overflow-y-auto">
                        {tasks.filter(t => t.status === col.id).map(task => {
                            ...
                            const assigneeName = task.assignedTo ? membersMap[task.assignedTo] : 'غير مسند';
                            return (
                                <div
                                    key={task.id}
                                    draggable
                                ...
                                >
                                    <h4 className="font-medium text-slate-800 dark:text-slate-100 text-sm mb-2">{task.title}</h4>
                                    ...
                                    <div className="flex items-center justify-between mt-4">
                                        <div className="flex items-center space-x-2 rtl:space-x-reverse">
                                            ...
                                            <span className="text-xs text-slate-500 dark:text-slate-400 truncate max-w-[100px]">{assigneeName}</span>
                                        </div>
                                        
                                        {task.dueDate && (
                                            <span className={`text-[10px] px-2 py-1 rounded-md ${isOverdue ? 'bg-amber-50 text-amber-600 dark:bg-amber-900/30' : 'bg-slate-50 text-slate-500 dark:bg-slate-700 dark:text-slate-400'}`}>
                                                {format(new Date(task.dueDate), 'd MMM', { locale: arSA })}
                                            </span>
                                        )}
                                    </div>
                                </div>
