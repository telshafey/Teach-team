...
export const ProjectCard: React.FC<ProjectCardProps> = React.memo(
  ({ project, onSelect, currency }) => {
    ...
    const budgetUsage = project.budgetHours
      ? (hoursLogged / project.budgetHours) * 100
      : 0;

    return (
      <div
        onClick={() => onSelect(project.id)}
        className="bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm p-4 space-y-3 cursor-pointer hover:shadow-md hover:border-sky-300 dark:hover:border-sky-700 transition-all"
      >
          ...
          {project.budgetNotificationSent && (
            <div className="flex items-center space-x-1 rtl:space-x-reverse text-amber-600 dark:text-amber-400 text-xs mt-2">
              <ExclamationTriangleIcon
                className="w-4 h-4"
                title={`تم إرسال إشعار بتجاوز ${project.budgetNotificationSent}% من الميزانية`}
              />
