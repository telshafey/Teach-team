                ...
                <div className="flex sticky top-0 bg-white dark:bg-slate-800 z-10 h-8 border-b">
                    {[...Array(totalDays)].map((_, i) => {
                        ...
                        if(date.getDate() === 1 || i === 0) {
                             return <div key={i} className="absolute text-xs font-semibold" style={{ right: `${i * 30}px`}}>{format(date, 'MMM yyyy', { locale: arSA })}</div>
                        }
                        return null;
                    })}
                </div>
                ...
                <div className="mt-2 space-y-2">
                    {ganttTasks.map((task, index) => (
                        <div key={task.id} className="flex items-center h-8">
                            <div className="w-32 truncate text-right pr-2 absolute -right-32">{task.title}</div>
                            <div className="absolute h-6 bg-sky-500 rounded" style={{
