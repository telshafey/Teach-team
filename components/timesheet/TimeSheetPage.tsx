...
import {
  ...
  isSameDay,
  isToday,
  isThisWeek as isWithinThisWeek,
  startOfMonth,
  endOfMonth,
  isWithinInterval,
  isFuture,
...
export const TimeSheetPage: React.FC<TimeSheetPageProps> = ({
  ...
  const calendarEvents = useMemo(() => {
    ...
    const dateMap: {
      [key: string]: { hasLog?: boolean; isDueDate?: boolean };
    } = {};
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
        <div>
          ...
          <p className="text-md text-slate-500 dark:text-slate-400">
            تتبع ساعات عملك اليومية والأسبوعية.
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-6">
        <Card className="text-center">
          <p className="text-2xl font-bold text-sky-600 dark:text-sky-400">
            {timeSheetSummary.todayHours.toFixed(1)}
          </p>
        ...
        <Card className="text-center">
          <p className="text-2xl font-bold text-sky-600 dark:text-sky-400">
            {timeSheetSummary.thisWeekHours.toFixed(1)}
          </p>
          <p className="text-sm font-semibold text-slate-500 dark:text-slate-400 mt-1">
            ساعات هذا الأسبوع
          </p>
        ...
        <Card className="text-center">
          <p className="text-2xl font-bold text-sky-600 dark:text-sky-400">
            {timeSheetSummary.thisMonthHours.toFixed(1)}
          </p>
          <p className="text-sm font-semibold text-slate-500 dark:text-slate-400 mt-1">
            ساعات هذا الشهر
          </p>
