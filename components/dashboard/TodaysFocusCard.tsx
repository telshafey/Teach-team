  ...
  >
    <div>
      ...
      <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
        ...
        {task.dueDate &&
          ` • تنتهي في: ${format(parseISO(task.dueDate), "d MMM", { locale: arSA })}`}
      </p>
    </div>
    {member && (
      <img
      ...
      {hasTasks ? (
        <div className="space-y-4">
              ...
              <div className="space-y-2">
                {inProgressTasks.map((task) => (
                  <TaskItem
                    ...
                    task={task}
                    project={
                      task.projectId ? projectsMap[task.projectId] : undefined
                    }
                    member={
          ...
          {dueTodayTasks.length > 0 && (
            <div>
              <h4 className="text-xs font-bold uppercase text-amber-600 dark:text-amber-400 mb-2">
                تستحق اليوم
              </h4>
              <div className="space-y-2">
