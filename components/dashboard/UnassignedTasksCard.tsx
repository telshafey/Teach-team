            ...
            <div className="flex-1 flex flex-col h-full">
                {tasks.length > 0 ? (
                    <div className="space-y-3 overflow-y-auto pr-1 pb-2">
                        {tasks.map(task => (
                            <div key={task.id} className="p-2 bg-slate-50 dark:bg-slate-700/50 border border-slate-100 dark:border-slate-700 rounded-md flex justify-between items-center transition-colors">
                                <div>
                                ...
                                <button onClick={() => onAssign(task)} className="text-xs font-semibold text-sky-600 dark:text-sky-400 hover:underline px-2 py-1 bg-sky-50 dark:bg-sky-900/30 rounded-md">
                                    إسناد
                                </button>
                            </div>
                        ))}
