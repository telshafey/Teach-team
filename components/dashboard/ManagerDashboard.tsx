...
const StatCardsWidget: React.FC<{
  data: { pending: number; hours: number; overdue: number; unassigned: number };
  onNavigate: (view: any, props?: any) => void;
}> = ({ data, onNavigate }) => (
  <div className="grid grid-cols-2 md:grid-cols-4 gap-6 h-full">
    <StatCard
      onClick={() => onNavigate("approvals")}
      icon={<BellIcon className="w-8 h-8 text-sky-500" />}
      label="موافقات معلقة"
      value={data.pending}
    />
    <StatCard
    ...
    />
    <StatCard
      icon={<ClipboardDocumentListIcon className="w-8 h-8 text-green-500" />}
...
const TeamActivityWidget: React.FC<{ team: any[]; tasks: Task[] }> = ({
  team,
  tasks,
}) => (
    ...
    <div className="space-y-3 flex-1 overflow-y-auto pr-1 pb-2">
      {team.map((member) => {
        const memberTasks = tasks.filter(
          (t) => t.assignedTo === member.id && t.status === "inprogress",
        );
        return (
          <div
            key={member.id}
...
const PendingApprovalsWidget: React.FC<{
  ...
  <Card
    ...
    headerActions={
      <a
        onClick={() => onNavigate("approvals")}
        className="text-sm font-semibold text-sky-600 dark:text-sky-400 hover:underline cursor-pointer"
      >
        عرض الكل
      </a>
    }
...
export const ManagerDashboard: React.FC = () => {
  ...
  const dashboardData = useMemo(() => {
    const myTeam = teamMembers.filter(
      (m) => m.id !== currentUser?.id && visibleMemberIds.has(m.id),
    );
    const myProjects = projects.filter((p) =>
      p.members?.some((m) => visibleMemberIds.has(m.teamMemberId)),
    );
    ...
    const teamLogsToday = dailyLogs.filter(
      (l) =>
        visibleMemberIds.has(l.teamMemberId) &&
        isSameDay(parseISO(l.date), now),
    );
  ...
  return (
    <div className="p-6" dir="rtl">
      <div className="mb-6 flex flex-col sm:flex-row justify-between items-center gap-4">
        <div>
          ...
          <p className="text-md text-slate-500 dark:text-slate-400">
            مرحباً {currentUser?.name}، إليك نظرة على فريقك.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="h-[400px]">
          <UnassignedTasksCard
            tasks={dashboardData.unassignedTasks}
            onAssign={setTaskToAssign}
          />
        </div>
        <div className="h-[400px]">
