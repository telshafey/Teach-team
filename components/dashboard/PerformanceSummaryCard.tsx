...
import { ClockIcon, CheckCircleIcon, ArrowUpIcon, ArrowDownIcon } from '../ui/Icons';
        ...
        <div className="flex items-start space-x-4 rtl:space-x-reverse">
            ...
            <div>
                ...
                <p className="text-2xl font-bold text-slate-800 dark:text-slate-100">
                    {currentValue.toFixed(unit === 'h' ? 1 : 0)} <span className="text-lg font-medium">{unit}</span>
                </p>
                <div className={`flex items-center text-xs font-semibold mt-1 ${isPositive ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                    {isPositive ? <ArrowUpIcon className="w-4 h-4" /> : <ArrowDownIcon className="w-4 h-4" />}
                    <span>{Math.abs(percentageChange).toFixed(0)}%</span>
                    <span className="text-slate-400 font-normal mr-1 rtl:mr-0 rtl:ml-1">عن الشهر الماضي</span>
                </div>
            </div>
        </div>
    );
