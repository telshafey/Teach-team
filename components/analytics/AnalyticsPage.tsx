...
export const AnalyticsPage: React.FC = () => {
  ...
  const memberHoursData = useMemo(() => {
    return teamMembers
      .map((member) => ({
        label: member.name,
        value: filteredDailyLogs
          .filter((l) => l.teamMemberId === member.id)
          .reduce((sum, l) => sum + l.hours, 0),
      }))
      .filter((item) => item.value > 0);
  ...
  const dailyProductivityData = useMemo(() => {
    ...
    return Object.entries(productivityMap).map(
      ([date, hours]: [string, number]) => ({
    ...
    <div className="p-6">
      <div className="mb-6">
        ...
        <p className="text-md text-slate-500">
          نظرة عميقة على بيانات الأداء والإنتاجية.
        </p>
      </div>
        ...
        <div className="p-4 flex flex-col md:flex-row gap-4 items-center">
          <div className="flex-1">
            ...
            <input
              ...
              className="w-full mt-1 p-2 border border-slate-300 dark:border-slate-600 rounded-md text-sm bg-white dark:bg-slate-700"
            />
          </div>
          <div className="flex-1">
            <label className="text-sm font-medium text-slate-700 dark:text-slate-300">
          ...
          <button
            onClick={handleReset}
            className="mt-6 px-4 py-2 text-sm font-semibold text-slate-700 bg-slate-200 rounded-md hover:bg-slate-300"
          >
            إعادة تعيين
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card
          title="توزيع ساعات العمل على المشاريع"
          icon={<FolderIcon className="w-5 h-5" />}
        >
          <BarChart title="" data={projectHoursData} />
        </Card>
        <Card
          title="توزيع ساعات العمل على أعضاء الفريق"
