        ...
        <div className="flex-grow">
          ...
          {meeting.startTime && (
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1 space-x-2 rtl:space-x-reverse">
              ...
              {project && <span>•</span>}
              <span>
                {format(parseISO(meeting.startTime), "eeee, d MMM", {
                  locale: arSA,
                })}
              </span>
              <span>•</span>
              <span>
                {format(parseISO(meeting.startTime), "p", { locale: arSA })}
              </span>
            </p>
          )}
        </div>
      ...
      <div className="mt-4 pt-3 border-t border-slate-200 dark:border-slate-700">
        ...
        <div className="flex items-center">
          ...
          {participants.length === 0 && (
            <p className="text-xs text-slate-400">لا يوجد مشاركون.</p>
          )}
        </div>
      </div>
    </div>
  ...
  const sortedPastMeetings = useMemo(() => {
    ...
    sortableMeetings.sort((a, b) => {
      ...
      switch (sortConfig.key) {
        ...
        case "project":
          aValue = a.projectId
            ? projectsMap[a.projectId]?.name || "zzzz"
            : "zzzz";
          bValue = b.projectId
            ? projectsMap[b.projectId]?.name || "zzzz"
            : "zzzz";
          break;
        case "startTime":
  ...
  return (
    <div className="p-6">
      <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
        <div>
          ...
          <p className="text-md text-slate-500 dark:text-slate-400">
            جدولة الاجتماعات والانضمام إليها.
          </p>
        </div>
        <button
      ...
      <div className="space-y-6">
        <Card>
          <h3 className="p-4 font-semibold text-slate-800 dark:text-slate-200 border-b border-slate-200 dark:border-slate-700">
            الاجتماعات القادمة ({upcomingMeetings.length})
          </h3>
        ...
        <Card>
          <h3 className="p-4 font-semibold text-slate-800 dark:text-slate-200 border-b border-slate-200 dark:border-slate-700">
            أرشيف الاجتماعات ({pastMeetings.length})
          </h3>
                  ...
                  {currentPastMeetings.map((meeting) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        {meeting.projectId
                          ? projectsMap[meeting.projectId]?.name
                          : "عام"}
                      </td>
                      <td className="px-4 py-3">
                        {meeting.startTime
