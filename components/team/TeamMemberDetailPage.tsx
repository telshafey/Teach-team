...
export const TeamMemberDetailPage: React.FC<TeamMemberDetailPageProps> = ({
  ...
  const memberData = useMemo(() => {
    ...
    const memberLogs = dailyLogs.filter(
      (log) => log.teamMemberId === member.id,
    );
    ...
    const currentMonthTasks = memberTasks.filter(
      (t) =>
        t.status === "done" &&
        t.dueDate &&
        isWithinInterval(parseISO(t.dueDate), {
          start: startOfCurrentMonth,
          end: endOfCurrentMonth,
        }),
    ).length;

    const lastMonthTasks = memberTasks.filter(
      (t) =>
        t.status === "done" &&
        t.dueDate &&
        isWithinInterval(parseISO(t.dueDate), {
