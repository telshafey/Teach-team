      ...
      <div className="flex items-center my-4">
        ...
        <div
          className={`org-card p-2 rounded-lg border-2 w-48 text-center shadow-md relative transition-all ${
            isSelected ? 'border-sky-500 bg-sky-50 dark:bg-sky-900/50' : 'border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700'
        ...
        >
          <img src={member.avatarUrl} alt={member.name} className="w-16 h-16 rounded-full mx-auto mb-2 ring-2 ring-white dark:ring-slate-700" />
          <p className="font-bold text-sm text-slate-800 dark:text-slate-100 truncate">{member.name}</p>
          <p className="text-xs text-slate-500 dark:text-slate-400">{role?.name || 'Unknown Role'}</p>
        </div>
      </div>

      {/* Vertical connector from parent */}
      <div className={`absolute top-0 right-4 rtl:left-4 w-px bg-slate-300 dark:bg-slate-600 ${isLast ? 'h-10' : 'h-full'}`}></div>
...
export const TeamOrgChart: React.FC<TeamOrgChartProps> = ({ members, onMemberClick, selectedMemberId, onMoveMember, canManage }) => {
          ...
          return (
            <div key={member.id} className="my-4">
              ...
              <div
                ...
                onClick={() => onMemberClick(member.id)}
                draggable={canManage}
                onDragStart={(e) => { if (!canManage) return; e.dataTransfer.setData("memberId", member.id.toString()); e.dataTransfer.effectAllowed = "move"; e.stopPropagation(); }}
              >
                  <img src={member.avatarUrl} alt={member.name} className="w-16 h-16 rounded-full mx-auto mb-2 ring-2 ring-white dark:ring-slate-700" />
                  <p className="font-bold text-sm text-slate-800 dark:text-slate-100 truncate">{member.name}</p>
                  <p className="text-xs text-slate-500 dark:text-slate-400">{role?.name || 'Unknown Role'}</p>
