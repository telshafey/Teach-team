...
export const GlobalSearch: React.FC = () => {
  ...
  const allCommands: Omit<Command, "permission">[] = [
    {
      ...
      action: () => onNavigate("projects", { isModalOpen: true }),
    },
    {
      id: "add-log",
      label: "إضافة سجل عمل",
      icon: <PlusIcon className="w-5 h-5" />,
      action: () => onNavigate("timesheet", { openLogModal: true }),
    },
    {
      id: "schedule-meeting",
      label: "جدولة اجتماع جديد",
      icon: <VideoCameraIcon className="w-5 h-5" />,
      action: () => onNavigate("meetings", { openMeetingModal: true }),
    },
    {
      id: "go-to-profile",
      label: "ملفي الشخصي",
    ...
    () =>
      allCommands
        .map((cmd) => {
          ...
          if (cmd.id === "add-project")
            permission = hasPermission("manage_projects");
          else if (cmd.id === "schedule-meeting")
            permission = !!siteSettings?.isMeetingsModuleEnabled;
          else if (cmd.id === "go-to-settings")
            permission =
        ...
        .filter((cmd) => cmd.permission),
    [allCommands, hasPermission, siteSettings],
  );
  ...
  const performSearch = useCallback(
    async (term: string) => {
      ...
      setIsLoading(true);
      try {
        const searchResults = await api.performGlobalSearch(
          supabaseClient,
          term,
        );
  ...
  return (
    <>
      ...
      >
        ...
        <kbd className="hidden md:inline text-xs font-sans font-semibold text-slate-400 border border-slate-300 dark:border-slate-600 rounded px-1.5 py-0.5">
          Ctrl+K
        </kbd>
      </button>
      ...
      {isOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center pt-20">
          <div
            ...
            className="bg-white dark:bg-slate-800 rounded-lg shadow-xl w-full max-w-lg max-h-[70vh] flex flex-col"
          >
              ...
              {filteredCommands.length > 0 || hasResults ? (
                <div className="p-2 space-y-2">
                  {filteredCommands.length > 0 && (
                    <div>
                      <h3 className="text-xs font-semibold uppercase text-slate-400 mb-1 px-2">
                        الإجراءات
                      </h3>
                      {filteredCommands.map((cmd, index) => (
                        <div
                          ...
                          className={`flex items-center space-x-3 rtl:space-x-reverse p-2 rounded-md cursor-pointer ${activeIndex === index ? "bg-slate-100 dark:bg-slate-700" : ""}`}
                        >
                          {cmd.icon}{" "}
                      ...
                      {results.projects.map((p, index) => {
                        ...
                        return (
                          <div
                            ...
                            onMouseMove={() => setActiveIndex(overallIndex)}
                            className={`flex items-center space-x-3 rtl:space-x-reverse p-2 rounded-md cursor-pointer ${activeIndex === overallIndex ? "bg-slate-100 dark:bg-slate-700" : ""}`}
                          >
                ...
                !isLoading && (
                  <p className="text-center text-slate-500 p-8">
                    اكتب للبحث أو اختر إجراءً سريعًا.
                  </p>
                )
              )}
            </div>
