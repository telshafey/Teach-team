...
export const generateProjectsSummary = (
    ...
    const headers = ["المشروع", "الحالة", "إجمالي الساعات المسجلة", "التكلفة التقديرية"];
    const rows = projects.map(p => {
        const projectLogs = dailyLogs.filter(l => l.projectId === p.id && dateFilter(l.date, filters));
...
export const generateTasksDetail = (
    ...
    const rows = tasksToReport.map(t => {
        ...
        return [t.title, project?.name || '', assignee?.name || 'غير مسندة', t.status, t.dueDate || ''];
    });
    return { headers, rows };
};
...
export const generateExpenses = (
    ...
    const totalAmount = claimsToReport.reduce((sum, claim) => sum + claim.amount, 0);
